...
for i in range(len(con_out)):
    ...
    if con_out[i] == non_con_out[i] or con_out_mean[i] == con_out[i] or con_out_mean[i] == non_con_out[i]:
        any_two_matches += 1
        match = "Match"
        if con_out[i] == non_con_out[i]: matched_element = con_out[i]
        elif con_out_mean[i] == con_out[i]: matched_element = con_out[i]
        elif con_out_mean[i] == non_con_out[i]: matched_element = con_out_mean[i]

    writer.writerow({'speaker': utt_Speaker_train[i].encode("utf-8"),
...
print("Matches in all lists(3): {}% and in context lists(2): {}%, any two matches: {}%".format(
    round((total_match/(i+1))*100, 2), round((con_matches/(i+1))*100, 2), round((any_two_matches/(i+1))*100, 2)))
