...
def main():
   ...
   #  print("Test results for non-context model - accuracy: {}".format(evaluation[1]))
    ...
    ckpts = ModelCheckpoint(con_model_name, monitor='val_loss', verbose=0, save_best_only=True)

    context_model.fit(X_train_con, Y_train_con, epochs=50, verbose=2, validation_split=0.15, callbacks=[ckpts])
